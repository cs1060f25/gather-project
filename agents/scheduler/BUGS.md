...
AI Attempts
N/A - Requires manual configuration
...
Frequency: Always
Predictability: Always occurs when attempting live tests
Desired behavior: Proper OAuth flow and token management for testing with real Google Calendar

Triage
- Severity: sev1-bug
- Priority: High (blocks testing of core functionality)

Tests
- Current test suite in calendar-test.ts
- All tests running in mock mode

Fixes
Proposed solutions:
1. Implement proper OAuth flow (auth.ts started but incomplete)
2. Add token management system
3. Add proper error handling for auth failures

AI Attempts
1. Initial OAuth setup attempted but facing configuration issues
2. Need to properly handle token refresh and storage

Review
Pending implementation and review

### BUG-002: Missing Timezone Validation
Description: Calendar events accept any string as timezone without validation
Impact: Could lead to invalid events or timezone mismatches
Steps to Reproduce:
1. Create event with invalid timezone string
2. Observe no validation error

Environment: Same as above
Frequency: Would occur whenever invalid timezone provided
...
Fixes
Proposed:
1. Add timezone validation using proper timezone database
...
[Additional bugs will be documented as testing continues]
