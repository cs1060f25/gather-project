...
async function runTests() {
  ...
  try {
    ...
    console.log(`Slots within 9AM-6PM: ${result1.slots.every(s => {
  ...
  try {
    ...
    const hasEveningSlots = result2.slots.some(s => {
      const hour = new Date(s).getUTCHours();
  ...
  try {
    ...
    const allLater = more.slots.every(newSlot => 
      initial.slots.every(oldSlot => newSlot > oldSlot)
    );
  ...
  try {
    ...
    const hasLunchHour = result4.slots.some(s => {
      const hour = new Date(s).getUTCHours();
  ...
  try {
    ...
    const hasJamie = result7.who.some(name => name.toLowerCase().includes("jamie"));
    const hasSpangler = result7.where?.toLowerCase().includes("spangler");
    const isLunchTime = result7.slots.some(s => {
      const hour = new Date(s).getUTCHours();
  ...
  try {
    ...
    const hasEveningSlots = result8.slots.some(s => {
      const hour = new Date(s).getUTCHours();
      return hour >= 18;
    });
